...
- **The core and the runtime are consumed packages now** (MIG-1 and MIG-2,
  ...
  [`motionRecording`](https://github.com/animu-sphere/usd-motion-plugins/blob/main/libs/motionRecording) instead, each pinned by
  **archive digest per target** in its descriptor, from that repository's
  v0.5.0 pin table.
  - **One library became two, and the split is visible in the link lines.** A
    member that samples links `motionSampling`; a member that reads or writes a
    capture trace links `motionRecording`; most link both.
  - **The vocabulary is the ecosystem's.** `motion::HumanoidPose` is
    `openstrata::motion::MotionPose`, `HumanoidAnimation` is `MotionClip`,
    `HumanBone` is `HumanJoint`, `ExpressionWeights` is `MotionChannelSet` and
    a pose's expression set is its `channels`. 104 files here were adapted to
    it; nothing about what they compute changed.
  - **Two behaviours changed with the package, and the exec bundle now reads
    them rather than predicting them.** `BlendPoses` answers `std::nullopt`
    where no source carries weight, so `motion.blendPoses` reads the optional
    instead of scanning the weights first; and a NaN weight now counts as *no
    weight* rather than poisoning the blend, which is recorded in the suite
    that used to assert the poisoning.
  - A pose's provenance is always present: what said "this sample stamped
    nothing" as an absent optional says it as a default `SourceMetadata` now.
  - **What this repository keeps of the fixtures those libraries held**: three
    capture traces under `tests/motion/fixtures/traces/`, because
    `motion_capture`'s replay suite reads them and a consumed package ships no
    test data. Two are format 3 and prove the consumed reader still accepts the
    version the corpus left at; the third is format 4, because the suite asks
    it to be what the consumed writer emits.
  - `motion.core` and `motion.runtime` are gone from the CTest label contract:
    a label whose only source directory left is not a label this workspace can
...
- **Two comparisons for one motion value, because three callers wanted
  different answers** — [`Compare.h`](https://github.com/animu-sphere/usd-motion-plugins/blob/main/libs/motionCore/include/motionCore/Compare.h)
...
- **A motion corpus**
  ([`libs/motionRuntime/tests/corpus`](https://github.com/animu-sphere/usd-motion-plugins/blob/main/libs/motionRecording/tests/corpus/README.md)):
