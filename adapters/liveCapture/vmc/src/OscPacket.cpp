...
{
public:
    Decoder(OscPacket* packet, Diagnostic* diagnostic)
        : _packet(packet)
    ...
    // `base` is the element's offset within the datagram, so a diagnostic
...
private:
    ...
    {
        if (_diagnostic) {
            *_diagnostic = MakeDiagnostic(
                DiagnosticCode::PacketMalformed,
                std::move(detail) + " (at byte " + std::to_string(offset)
                    + ")");
            _diagnostic->subject.assign(subject);
        }
        return false;
    }
                ...
                Diagnostic* diagnostic)
{
    if (!packet) {
        if (diagnostic) {
            *diagnostic = MakeDiagnostic(DiagnosticCode::PacketMalformed,
    ...
    if (size != 0 && !bytes) {
        if (diagnostic) {
            *diagnostic = MakeDiagnostic(DiagnosticCode::PacketMalformed,
                                         "a datagram of " + std::to_string(size)
                                             + " bytes has no bytes");
        }
        return false;
    }
    ...
    OscPacket result;
    Decoder decoder(&result, diagnostic);
