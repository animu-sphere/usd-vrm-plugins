...
{
    return {
        0x2f, 0x56, 0x4d, 0x43, 0x2f, 0x45, 0x78, 0x74, 0x2f, 0x54, 0x00, 0x00,
        0x2c, 0x66, 0x00, 0x00,
        0x3d, 0xcc, 0xcc, 0xcd,
    };
}
...
TestRoundTripIsByteIdentical()
{
    ...
    capture.datagrams.push_back(
        Datagram(0.033333, {0x23, 0x62, 0x75, 0x6e, 0x64, 0x6c, 0x65, 0x00,
...
TestAGutterIsCheckedRatherThanSkipped()
{
    ...
    assert(parsedUpper.datagrams[0].bytes
           == std::vector<std::uint8_t>({0x2f, 0x56, 0x4d, 0x43}));
...
CheckCorpus(const std::filesystem::path& directory)
{
    ...
    for (const std::filesystem::path& path : captures) {
        ...
        if (!vrmAdapterVmc::WritePacketCapture(rewritten, parsed)
