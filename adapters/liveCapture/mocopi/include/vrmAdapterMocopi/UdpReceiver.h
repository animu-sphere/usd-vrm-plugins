...
// can deliver, and the same bound the capture format enforces. Truncation is
// therefore impossible rather than configurable, which is a decision about
// blame: this adapter's whole purpose is to find out what a source emits, and a
// smaller buffer would let the receiver manufacture a malformed packet against
// a source that sent a perfectly good one. If some transport ever does deliver
// more, the datagram is counted in `datagramsTruncated` and dropped, never
// passed on half-read.
//
...
struct UdpReceiverStats
{
    ...
    // buffer *is* the maximum, and only Windows would report it in any case:
    // POSIX truncates silently, so a short read there is indistinguishable from
    // a whole datagram.
...
private:
    ...
    // an accepted datagram, which is what makes the report one per episode.
