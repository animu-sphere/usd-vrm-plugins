...
struct ExactPolicy
{
    ...
    static bool
    Time(double a, double b) noexcept
    {
        return a == b;
    }
    static bool
    Confidence(float a, float b) noexcept
    {
        return a == b;
    }
    static bool
    ...
    static bool
    Vector(const pxr::GfVec3f& a, const pxr::GfVec3f& b) noexcept
    {
        return a[0] == b[0] && a[1] == b[1] && a[2] == b[2];
    }

    static bool
    Rotation(const pxr::GfQuatf& a, const pxr::GfQuatf& b) noexcept
    {
        return a.GetReal() == b.GetReal() && Vector(a.GetImaginary(), b.GetImaginary());
    }
};
...
struct TolerantPolicy
{
    ...
    static bool
    Within(double difference, double limit) noexcept
    {
        return std::abs(difference) <= limit;
    }

    bool
    Time(double a, double b) const noexcept
    {
        return Within(a - b, tolerance.time);
    }

    bool
    Confidence(float a, float b) const noexcept
    {
    ...
    bool
    Vector(const pxr::GfVec3f& a, const pxr::GfVec3f& b) const noexcept
    {
        return Within((a - b).GetLength(), tolerance.distance);
    }

    bool
    Velocity(const pxr::GfVec3f& a, const pxr::GfVec3f& b) const noexcept
    {
        return Within((a - b).GetLength(), tolerance.velocity);
    }

    bool
    Rotation(const pxr::GfQuatf& a, const pxr::GfQuatf& b) const noexcept
    {
...
bool
CompareVelocity(const ExactPolicy&, const pxr::GfVec3f& a, const pxr::GfVec3f& b) noexcept
{
    return ExactPolicy::Vector(a, b);
}

bool
CompareVelocity(const TolerantPolicy& policy, const pxr::GfVec3f& a, const pxr::GfVec3f& b) noexcept
{
...
bool
CompareRoot(const Policy& policy, const RootMotion& a, const RootMotion& b, std::string* difference)
{
    if (a.hasPosition != b.hasPosition || a.hasOrientation != b.hasOrientation ||
        a.hasLinearVelocity != b.hasLinearVelocity || a.hasAngularVelocity != b.hasAngularVelocity)
    {
        Report(difference, [] { return std::string("root carries different fields"); });
        return false;
    }
    ...
    if (a.hasOrientation && !policy.Rotation(a.worldOrientation, b.worldOrientation))
    {
        Report(difference,
               [&]
               {
                   return "root orientation " +
                          Amount(AngleDifference(a.worldOrientation, b.worldOrientation), "rad");
               });
        return false;
    }
    if (a.hasLinearVelocity && !CompareVelocity(policy, a.linearVelocity, b.linearVelocity))
    {
        Report(difference,
               [&]
               {
                   return "root linear velocity " +
                          Amount((a.linearVelocity - b.linearVelocity).GetLength(), "m/s");
               });
        return false;
    }
    if (a.hasAngularVelocity && !CompareVelocity(policy, a.angularVelocity, b.angularVelocity))
    {
        Report(difference,
               [&]
               {
                   return "root angular velocity " +
                          Amount((a.angularVelocity - b.angularVelocity).GetLength(), "rad/s");
               });
        return false;
    }
    return true;
            ...
            std::string* difference)
{
    ...
    {
        Report(difference, [&] { return "timestamp " + Amount(a.timestamp - b.timestamp, "s"); });
        return false;
    }
    ...
    {
        Report(difference, [] { return std::string("only one pose reports confidence"); });
        return false;
    }
    if (a.confidence)
    {
    ...
    {
        Report(difference, [] { return std::string("only one pose reports foot contact"); });
        return false;
    }
    ...
    {
        Report(difference, [] { return std::string("only one pose reports a look-at target"); });
        return false;
    }
                 ...
                 std::string* difference)
{
    ...
    {
        Report(difference, [&] { return "startTime " + Amount(a.startTime - b.startTime, "s"); });
        return false;
    }
    if (!policy.Time(a.endTime, b.endTime))
    {
        Report(difference, [&] { return "endTime " + Amount(a.endTime - b.endTime, "s"); });
        return false;
    }
...
bool
NearlyEqual(const RootMotion& a, const RootMotion& b, const MotionTolerance& tolerance,
            std::string* difference)
{
