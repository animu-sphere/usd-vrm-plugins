...
void
AppendU32(std::vector<std::byte>& out, std::uint32_t value)
{
...
vc::ByteView
View(const std::vector<std::byte>& bytes)
{
    return {bytes.data(), bytes.size()};
}

void
SetU32(std::vector<std::byte>& bytes, std::size_t offset, std::uint32_t value)
{
    for (unsigned shift = 0; shift < 32; shift += 8)
    {
...
TestMalformedContainers()
{
    ...
    bytes.insert(bytes.end(), {std::byte{5}, std::byte{6}, std::byte{7}, std::byte{8}});
    SetU32(bytes, 8, static_cast<std::uint32_t>(bytes.size()));
    CHECK(!vc::ParseGlb(View(bytes), &glb, &error));
...
TestBufferViews()
{
    ...
    CHECK(!vc::MakeBufferView(View(bytes), 1, std::numeric_limits<std::size_t>::max(), &slice,
                              &error));
}
...
TestResourceNaming()
{
    const std::vector<std::byte> bytes = {std::byte{0x89}, std::byte{'P'}, std::byte{'N'},
                                          std::byte{'G'}};
    CHECK(vc::MakeEmbeddedResourcePath(View(bytes), "png") == "images/dfa761ae3f638411.png");
