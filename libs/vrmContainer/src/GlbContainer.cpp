...
std::uint32_t
ReadU32(const std::byte* bytes) noexcept
{
...
bool
Fail(ErrorCode code, std::size_t offset, Error* error) noexcept
{
    if (error)
    {
        error->code = code;
...
bool
HasGlbMagic(ByteView bytes) noexcept
{
    return bytes.valid() && bytes.size() >= 4 && ReadU32(bytes.data()) == GlbMagic;
}

bool
MakeBufferView(ByteView buffer, std::size_t offset, std::size_t length, ByteView* out,
               Error* error) noexcept
{
    ...
    if (out)
        *out = {};
    if (!out)
        return Fail(ErrorCode::InvalidArgument, 0, error);
    if (!buffer.valid())
        return Fail(ErrorCode::NullData, 0, error);
...
bool
ParseGlb(ByteView bytes, GlbView* out, Error* error) noexcept
{
    if (error)
        *error = {};
    if (out)
...
const char*
ErrorMessage(ErrorCode code) noexcept
{
    switch (code)
    {
    case ErrorCode::None:
    ...
    case ErrorCode::InvalidArgument:
        return "required output pointer is null";
    case ErrorCode::NullData:
        return "non-empty byte view has null data";
    case ErrorCode::HeaderTooShort:
        return "GLB header is truncated";
    case ErrorCode::InvalidMagic:
        return "GLB magic is not glTF";
    case ErrorCode::UnsupportedVersion:
        return "GLB version is not 2";
    case ErrorCode::LengthMismatch:
        return "GLB declared length does not match input";
    case ErrorCode::ChunkHeaderTruncated:
        return "GLB chunk header is truncated";
    case ErrorCode::ChunkLengthUnaligned:
        return "GLB chunk length is not 4-byte aligned";
    case ErrorCode::ChunkOutOfRange:
        return "GLB chunk extends beyond the container";
    case ErrorCode::JsonChunkMissing:
        return "GLB JSON chunk is missing";
    case ErrorCode::JsonChunkNotFirst:
        return "GLB JSON chunk is not first";
    case ErrorCode::DuplicateBinChunk:
        return "GLB has more than one BIN chunk";
    case ErrorCode::ByteRangeOutOfRange:
        return "byte range extends beyond its buffer";
    }
    return "unknown vrmContainer error";
}

std::uint64_t
HashBytes(ByteView bytes) noexcept
{
    if (!bytes.valid())
        return 0;
...
std::string
MakeEmbeddedResourcePath(ByteView bytes, const char* extension)
{
    if (!bytes.valid() || !extension || !*extension)
        return {};
    char hash[17];
    std::snprintf(hash, sizeof(hash), "%016llx", static_cast<unsigned long long>(HashBytes(bytes)));
