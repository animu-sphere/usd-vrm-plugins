...
// for the same reason.
template <typename Enum> struct Term
{
    Enum value;
...
constexpr std::array<Term<SourceHandedness>, SourceHandednessCount> kHandedness = {{
    {SourceHandedness::Unspecified, "unspecified", {}},
    {SourceHandedness::Right, "right", {}},
    {SourceHandedness::Left, "left", {}},
}};
static_assert(TermsAreInEnumOrder(kHandedness), "kHandedness must be in enumerator order");
...
static_assert(TermsAreInEnumOrder(kAxes), "kAxes must be in enumerator order");

constexpr std::array<Term<SourceLengthUnit>, SourceLengthUnitCount> kLengthUnits = {{
...
static_assert(TermsAreInEnumOrder(kLengthUnits), "kLengthUnits must be in enumerator order");
...
constexpr std::array<Term<RootRotationPolicy>, RootRotationPolicyCount> kRootRotationPolicies = {{
...
constexpr std::array<Term<RestPoseSource>, RestPoseSourceCount> kRestPoseSources = {{
...
constexpr std::array<Term<UnmappedJointPolicy>, UnmappedJointPolicyCount> kUnmappedJointPolicies = {
    {
...
constexpr std::array<Term<SourceProfileRefusal>, SourceProfileRefusalCount> kRefusals = {{
...
static_assert(TermsAreInEnumOrder(kRefusals), "kRefusals must be in enumerator order");
...
{
    if (ancestor >= skeleton.joints.size() || descendant >= skeleton.joints.size())
    {
        return false;
    }
...
{
    return axis == SourceAxis::MinusX || axis == SourceAxis::MinusY || axis == SourceAxis::MinusZ;
}
...
bool
operator==(const SourceJointMapping& lhs, const SourceJointMapping& rhs) noexcept
{
    return lhs.sourceName == rhs.sourceName && lhs.bone == rhs.bone && lhs.required == rhs.required;
}
...
{
    return lhs.id == rhs.id && lhs.producer == rhs.producer && lhs.rootJoint == rhs.rootJoint &&
           lhs.handedness == rhs.handedness && lhs.upAxis == rhs.upAxis &&
           lhs.forwardAxis == rhs.forwardAxis && lhs.translationUnit == rhs.translationUnit &&
           lhs.rootTranslation == rhs.rootTranslation && lhs.rootRotation == rhs.rootRotation &&
           lhs.restPose == rhs.restPose && lhs.unmappedJoints == rhs.unmappedJoints &&
           lhs.joints == rhs.joints && lhs.ignoredJoints == rhs.ignoredJoints;
}
...
{
    return std::find(ignoredJoints.begin(), ignoredJoints.end(), sourceName) != ignoredJoints.end();
}
...
ValidateSourceProfile(const SourceProfile& profile, std::string* reason)
{
    ...
    if (*SourceAxisComponent(profile.upAxis) == *SourceAxisComponent(profile.forwardAxis))
    {
        ...
        return Fail(reason, std::string("profile's up axis ") +
                                std::string(SourceAxisName(profile.upAxis)) + " and forward axis " +
    ...
    for (std::size_t i = 0; i < profile.joints.size(); ++i)
    {
        ...
        for (std::size_t j = 0; j < i; ++j)
        {
            ...
            {
                return Fail(reason,
                            MappingLabel(i, mapping) + " maps a name this profile already maps");
            }
        ...
        {
            return Fail(reason, MappingLabel(i, mapping) + " is both mapped and ignored");
        }
    }
    ...
    for (std::size_t i = 0; i < profile.ignoredJoints.size(); ++i)
    {
        ...
        {
            char buffer[56];
            std::snprintf(buffer, sizeof(buffer), "ignored joint %zu has no name", i);
            return Fail(reason, buffer);
        }
        for (std::size_t j = 0; j < i; ++j)
        {
            ...
            {
                return Fail(reason,
                            "ignored joint '" + profile.ignoredJoints[i] + "' is listed twice");
            }
        }
    }
    ...
    {
        return Fail(reason, MappingLabel(*hips, profile.joints[*hips]) +
...
MatchSourceProfile(const SourceProfile& profile, const SourceSkeleton& skeleton)
{
    ...
    for (const SourceJointMapping& mapping : profile.joints)
    {
        ...
        const std::vector<std::size_t> found = skeleton.FindJoints(mapping.sourceName);
        if (found.size() > 1)
        {
        ...
        match.bound.push_back(SourceProfileBinding{mapping.bone, found[0], mapping.required});
    ...
    else if (!match.rootMatched)
    {
        ...
        match.detail = "profile roots at '" + profile.rootJoint + "'; the rig roots at '" +
                       skeleton.joints[0].name + "'";
    }
    ...
    else if (!match.unmappedJoints.empty() && profile.unmappedJoints == UnmappedJointPolicy::Refuse)
    {
