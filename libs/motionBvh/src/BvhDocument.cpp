...
Fail(Diagnostic* diagnostic, std::string detail, std::string subject = {})
{
    if (diagnostic)
    {
        *diagnostic = MakeDiagnostic(DiagnosticCode::ParseFailed, std::move(detail));
...
ValidateBvhDocument(const BvhDocument& document, Diagnostic* diagnostic)
{
    ...
    for (std::size_t index = 0; index < document.joints.size(); ++index)
    {
        ...
        if (index > 0)
        {
            ...
            if (static_cast<std::size_t>(joint.parent) >= index)
            {
                return Fail(diagnostic,
                            "joint " + Count(index) +
                                " is stored before its "
                                "parent " +
                                Count(static_cast<std::size_t>(joint.parent)),
                            joint.name);
            }
        }
        if (joint.channelOffset != offset)
        {
            return Fail(diagnostic,
                        "joint " + Count(index) + " starts at column " +
                            Count(joint.channelOffset) +
                            ", declaration order puts "
                            "it at " +
                            Count(offset),
                        joint.name);
        }
        ...
        for (const BvhChannel channel : joint.channels)
        {
            ...
            {
                return Fail(diagnostic, "joint " + Count(index) + " carries an unknown channel",
                            joint.name);
            }
        }
        for (const float component : {joint.offset.x, joint.offset.y, joint.offset.z})
        {
            if (!std::isfinite(component))
            {
                ...
                {
                    *diagnostic =
                        MakeDiagnostic(DiagnosticCode::NonFiniteValue, "offset is not finite");
        ...
        if (joint.endSiteOffset)
        {
            for (const float component :
                 {joint.endSiteOffset->x, joint.endSiteOffset->y, joint.endSiteOffset->z})
            {
                if (!std::isfinite(component))
                {
                    if (diagnostic)
    ...
    {
        return Fail(diagnostic, "values holds " + Count(document.values.size()) + " numbers, " +
                                    Count(document.frameCount) + " frames x " +
        ...
        (document.frameCount > 1 && document.frameTime <= 0.0))
    {
        if (diagnostic)
        {
            ...
            *diagnostic = MakeDiagnostic(DiagnosticCode::InvalidFrameTime,
