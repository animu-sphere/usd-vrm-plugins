...
{
  public:
    Parser(std::string_view text, const BvhParseOptions& options, Diagnostic* diagnostic)
        : scanner_(text), text_(text), options_(options), diagnostic_(diagnostic)
    {
    }
  ...
  private:
    bool
    Fail(DiagnosticCode code, std::size_t line, std::string subject, std::string detail)
    {
        if (diagnostic_)
        {
    ...
    bool
    FailAtEnd(std::string detail)
    {
        return Fail(DiagnosticCode::ParseFailed, scanner_.Line(), {}, std::move(detail));
    }
...
Parser::ExpectKeyword(std::string_view keyword)
{
    ...
    {
        return FailAtEnd("expected " + std::string(keyword) + ", reached the end of the file");
    }
    if (!EqualsIgnoreCaseAscii(token.text, keyword))
    {
        return Fail(DiagnosticCode::ParseFailed, token.line, std::string(token.text),
...
Parser::ExpectBrace(char brace)
{
    ...
    if (!scanner_.Next(&token))
    {
        return FailAtEnd(std::string("expected '") + brace + "', reached the end of the file");
    }
...
Parser::ExpectLabel(std::string_view word)
{
    ...
    if (!scanner_.Next(&token))
    {
        return FailAtEnd("expected " + std::string(word) + ", reached the end of the file");
    }
...
Parser::ReadOffset(BvhVec3* offset)
{
    ...
    for (std::size_t index = 0; index < 3; ++index)
    {
        ...
        {
            return Fail(DiagnosticCode::ParseFailed, token.line, std::string(token.text),
                        "OFFSET component " + Count(index + 1) + " is not a number");
        }
        if (!std::isfinite(value))
        {
            return Fail(DiagnosticCode::NonFiniteValue, token.line, std::string(token.text),
                        "OFFSET component " + Count(index + 1) + " is not finite");
        }
...
Parser::ReadChannels(BvhJoint* joint)
{
    ...
    {
        return Fail(DiagnosticCode::ParseFailed, countToken.line, std::string(countToken.text),
    ...
    for (std::size_t index = 0; index < declared; ++index)
    {
        ...
        if (!scanner_.Next(&token))
        {
            return FailAtEnd("CHANNELS declared " + Count(declared) +
                             ", reached the end of the file after " + Count(index));
        }
        ...
        if (EqualsIgnoreCaseAscii(token.text, "JOINT") ||
            ...
            EqualsIgnoreCaseAscii(token.text, "MOTION") || token.text == "}" || token.text == "{")
        {
            return Fail(DiagnosticCode::ParseFailed, token.line, std::string(token.text),
                        "CHANNELS declared " + Count(declared) + " channels for joint '" +
                            joint->name + "'; channel " + Count(index + 1) +
                            " is a structural keyword");
        }
        const std::optional<BvhChannel> channel = FindBvhChannel(token.text);
        if (!channel)
        {
            return Fail(DiagnosticCode::UnsupportedChannel, token.line, std::string(token.text),
                        "joint '" + joint->name +
                            "' declares a channel this "
...
Parser::ReadEndSite(BvhJoint* joint)
{
    ...
    {
        return FailAtEnd("joint '" + joint->name + "' carries more than one End Site");
    }
    if (!ExpectBrace('{'))
    {
        return false;
...
Parser::ReadJointBody(std::size_t jointIndex, std::size_t depth)
{
    if (depth > options_.limits.maxHierarchyDepth)
    {
        return FailAtEnd("hierarchy is deeper than the " +
                         Count(options_.limits.maxHierarchyDepth) + "-level limit");
    }
    if (!ExpectBrace('{'))
    {
        return false;
    ...
    for (;;)
    {
        ...
        if (!scanner_.Next(&token))
        {
            return FailAtEnd("joint '" + work_.joints[jointIndex].name + "' is not closed");
        }
        if (token.text == "}")
        {
            break;
        ...
        if (EqualsIgnoreCaseAscii(token.text, "OFFSET"))
        {
            if (haveOffset)
            {
                return Fail(DiagnosticCode::ParseFailed, token.line, work_.joints[jointIndex].name,
        ...
        if (EqualsIgnoreCaseAscii(token.text, "JOINT"))
        {
            ...
            {
                return Fail(DiagnosticCode::ParseFailed, name.line, std::string(name.text),
                            "JOINT needs a name");
            }
            if (work_.joints.size() >= options_.limits.maxJoints)
            {
                return Fail(DiagnosticCode::ParseFailed, name.line, std::string(name.text),
                            "hierarchy carries more than the " + Count(options_.limits.maxJoints) +
                                "-joint limit");
            }
            BvhJoint child;
    ...
    if (!haveOffset)
    {
        return FailAtEnd("joint '" + work_.joints[jointIndex].name + "' declares no OFFSET");
    }
...
Parser::ReadMotion()
{
    ...
    {
        return Fail(DiagnosticCode::ParseFailed, framesToken.line, std::string(framesToken.text),
                    "Frames: is not a non-negative integer");
    }
    if (declaredFrames > options_.limits.maxFrames)
    {
        return Fail(DiagnosticCode::ParseFailed, framesToken.line, std::string(framesToken.text),
                    "Frames: declares more than the " + Count(options_.limits.maxFrames) +
                        "-frame limit");
    }
    ...
    {
        return Fail(DiagnosticCode::InvalidFrameTime, timeToken.line, std::string(timeToken.text),
    ...
    {
        return Fail(DiagnosticCode::InvalidFrameTime, timeToken.line, std::string(timeToken.text),
                    "Frame Time: is zero across " + Count(declaredFrames) + " frames");
    }
    ...
    while (position < text_.size())
    {
        ...
        if (framesRead >= declaredFrames)
        {
            return Fail(DiagnosticCode::ParseFailed, rowLine, {},
                        "Frames: declared " + Count(declaredFrames) + ", the file carries more");
        }
        ...
        while (offset < row.size())
        {
            ...
            const std::string_view token = row.substr(tokenStart, offset - tokenStart);

            if (valuesInRow >= work_.channelCount)
            {
                return Fail(
                    DiagnosticCode::FrameWidthMismatch, rowLine, "frame " + Count(framesRead),
                    "expected " + Count(work_.channelCount) + " values, the row carries more");
            }
            double value = 0.0;
            if (!ParseDoubleToken(token, &value))
            {
                return Fail(DiagnosticCode::ParseFailed, rowLine, std::string(token),
                            "frame " + Count(framesRead) + " value " + Count(valuesInRow) +
                                " is not a number");
            }
            if (!std::isfinite(value))
            {
                return Fail(DiagnosticCode::NonFiniteValue, rowLine, std::string(token),
                            "frame " + Count(framesRead) + " value " + Count(valuesInRow) +
                                " is not finite");
            }
        ...
        if (valuesInRow != work_.channelCount)
        {
            return Fail(DiagnosticCode::FrameWidthMismatch, rowLine, "frame " + Count(framesRead),
                        "expected " + Count(work_.channelCount) + " values, read " +
                            Count(valuesInRow));
        }
        ++framesRead;
    }
    ...
    if (framesRead != declaredFrames)
    {
        return Fail(DiagnosticCode::ParseFailed, line, {},
                    "Frames: declared " + Count(declaredFrames) + ", read " + Count(framesRead));
    }
    return true;
}
...
Parser::Parse(BvhDocument* document)
{
    ...
    {
        return Fail(DiagnosticCode::ParseFailed, rootName.line, std::string(rootName.text),
                    "ROOT needs a name");
    }
...
bool
ParseBvhText(std::string_view text, BvhDocument* document, Diagnostic* diagnostic,
...
bool
ParseBvhFile(const std::filesystem::path& path, BvhDocument* document, Diagnostic* diagnostic,
             const BvhParseOptions& options)
{
    ...
    if (!file)
    {
        ...
        {
            *diagnostic =
                MakeDiagnostic(DiagnosticCode::ParseFailed, "the file could not be opened");
    ...
    if (file.bad())
    {
        if (diagnostic)
        {
            *diagnostic = MakeDiagnostic(DiagnosticCode::ParseFailed, "the file could not be read");
            diagnostic->source = resolved.source;
        }
        return false;
    }
