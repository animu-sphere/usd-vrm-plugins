                              ...
                              std::vector<std::string>* outWarnings) const
{
    ...
    for (size_t i = 0; i < doc.materials.size(); ++i)
    {
        ...
        // Canonical semantics first, on the Material (material policy §6).
        // Every rendering realization is then generated from what the stage
        // now carries -- read back, not handed over -- so each is a function
        // of the canonical attributes and of nothing the importer knows
        // besides (P5 Steps 5-6), and regenerating either on any stage gives
        // the same graph. /mtlx is generated from the semantics too, never
        // from /preview (§3).
        UsdVrmAuthorMaterialSemantics(mat, vm.semantics);
        const VrmMaterialSemantics semantics = UsdVrmReadMaterialSemantics(mat);
        UsdVrmAuthorPreview(mat, semantics);
        UsdVrmAuthorMtlx(mat, semantics);
