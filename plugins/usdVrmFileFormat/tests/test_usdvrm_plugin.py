...
def check_textures():
    ...
    # MToon preserved as metadata, and typed.
    assert mat.GetAttribute("vrm:shaderModel").Get() == "MToon"
    assert mat.GetCustomData().get("vrm", {}).get("mtoon", {}).get("raw")
    assert mat.GetAttribute("inputs:vrm:mtoon:shadeColorFactor").Get() ==         Gf.Vec3f(0.5, 0.2, 0.2)


def _materials_by_source_index(stage):
    out = {}
    for prim in stage.Traverse():
...
def check_material_semantics():
    """glTF / VRM 1.0 source -> canonical attributes, on every 1.0 fixture.

    Material policy §6 (P5 Step 4): VrmMaterialAPI on every material,
    VrmMToonAPI on an MToon one, one VrmTextureInfoAPI per texture, every value
    authored -- compared field by field against the source JSON, restated
    independently in material_oracle.
    """
    checked = 0
    for path in sorted(FIXTURES.glob("*.vrm")):
        glb = material_oracle.read_glb(path)
        if glb is None:
            continue  # invalid.vrm: not a GLB at all
        gltf, binary = glb
        if "VRMC_vrm" not in gltf.get("extensions", {}):
            continue  # VRM 0.x: proved by the mtoon_vrm0 / mtoon_vrm1 pair
        stage = _open(path.name)
        prims = _materials_by_source_index(stage)
        assert sorted(prims) == list(range(len(gltf.get("materials", [])))), path.name
        for index, prim in prims.items():
            material_oracle.assert_same(
                material_oracle.expected(gltf, binary, index),
                material_oracle.actual(prim), f"{path.name}:{prim.GetPath()}")
            checked += 1
    # textures, materials, the MToon pair and the rest: not a vacuous pass.
    assert checked >= 15, checked


def check_mtoon_vrm0_matches_vrm1():
    """VRM 0.x MToon normalizes into the same canonical values as 1.0 (§6.6).

    mtoon_vrm1.vrm is mtoon_vrm0.vrm as UniVRM's migration writes it, with each
    1.0 value worked out by hand; the two stages must agree on every schema,
    attribute and texture of every material.
    """
    vrm0, vrm1 = _open("mtoon_vrm0.vrm"), _open("mtoon_vrm1.vrm")
    names = [p.GetName() for p in vrm1.GetPrimAtPath("/Asset/mtl").GetChildren()]
    assert names == ["Hair", "Face", "Veil", "Tear", "Lashes", "Plain"], names
    for name in names:
        path = f"/Asset/mtl/{name}"
        material_oracle.assert_same(
            material_oracle.actual(vrm1.GetPrimAtPath(path)),
            material_oracle.actual(vrm0.GetPrimAtPath(path)), f"mtoon_vrm0:{path}")

    hair = vrm0.GetPrimAtPath("/Asset/mtl/Hair")
    # The canonical colour is materialProperties' _Color, sRGB-decoded -- not
    ...
    assert json.loads(raw) == gltf["extensions"]["VRM"]["materialProperties"][0]
    assert hair.GetAttribute("vrm:shaderModel").Get() == "MToon"
    # A 0.x material that is not MToon carries the glTF core only.
