...
`ost plugin build` 0.6+ generates the schema C++ in `.strata/` for the resolved
runtime. This script refreshes the committed fallback used by plain CMake and
the checked-in USD registration resources. It runs OpenUSD's `usdGenSchema`,
then folds the result into the vrmSchema bundle:

  * C++ (api.h, tokens.{h,cpp}, vrm*API.{h,cpp}) -> src/vrmSchema/  (committed,
    compiled into libvrmSchema; the Python bindings wrap*.cpp and module.cpp
    are intentionally skipped — the plugin ships no Python module).
  * generatedSchema.usda -> plugin/resources/vrmSchema/  (committed; USD's
    schema registry reads it next to plugInfo.json).
  * the generated schema `Types` block -> merged into the committed
    plugin/resources/vrmSchema/plugInfo.json and plugInfo.json.in.
...
CPP_DEST = PLUGIN_DIR / "src" / "vrmSchema"
RESOURCES = PLUGIN_DIR / "plugin" / "resources" / "vrmSchema"
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        for name in _cpp_files(gen):
            ...
            print(f"  src/vrmSchema/{name}")
        shutil.copy2(gen / "generatedSchema.usda", RESOURCES / "generatedSchema.usda")
        print("  plugin/resources/vrmSchema/generatedSchema.usda")

        gen_types = _read_schema_types(gen / "plugInfo.json")
        for target in (PLUGINFO, PLUGINFO_TEMPLATE):
...
def _merge_plug_info(target: Path, gen_types: dict) -> None:
    ...
    preserving the hand-written plugin entry (Name/LibraryPath/paths) and
    pruning any autoGenerated Types from a previous run that the current schema
    no longer defines, so a renamed/removed class doesn't linger."""
