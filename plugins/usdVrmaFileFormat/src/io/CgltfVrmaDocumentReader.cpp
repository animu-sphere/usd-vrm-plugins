...
const JsValue*
Find(const JsObject& object, const char* key)
{
...
const JsObject*
AsObject(const JsValue* value)
{
    return value && value->IsObject() ? &value->GetJsObject() : nullptr;
}

int
AsInt(const JsValue* value, int fallback = -1)
{
    return value && value->IsInt() ? value->GetInt() : fallback;
}

GfMatrix4d
NodeLocal(const cgltf_node& node)
{
    if (node.has_matrix)
    {
        ...
        return GfMatrix4d(m[0], m[1], m[2], m[3], m[4], m[5], m[6], m[7], m[8], m[9], m[10], m[11],
                          m[12], m[13], m[14], m[15]);
    }
    ...
    if (node.has_translation)
        std::memcpy(t, node.translation, sizeof(t));
    if (node.has_rotation)
        std::memcpy(r, node.rotation, sizeof(r));
    if (node.has_scale)
        std::memcpy(s, node.scale, sizeof(s));
...
// own origin.
bool
StatesPlacement(const cgltf_node& node)
{
...
GfMatrix4d
AncestorTransform(const cgltf_node& node)
{
    GfMatrix4d composed(1.0);
    for (const cgltf_node* ancestor = node.parent; ancestor; ancestor = ancestor->parent)
    {
...
void
ReadKey(const cgltf_animation_sampler* sampler, std::size_t key, int components, float* out)
{
...
void
Sample(const cgltf_animation_sampler* sampler, const std::vector<float>& keys, float time,
       int components, float* out)
{
    ...
    const std::size_t high =
        static_cast<std::size_t>(std::upper_bound(keys.begin(), keys.end(), time) - keys.begin());
    ...
    if (components == 4)
    {
        const GfQuatd rotation =
            GfSlerp(alpha, GfQuatd(a[3], a[0], a[1], a[2]), GfQuatd(b[3], b[0], b[1], b[2]));
...
bool
CgltfVrmaDocumentReader::Read(const std::string& resolvedPath, const std::vector<std::byte>& bytes,
                              VrmaCanonicalDocument* outDocument, std::string* outError)
{
    ...
    if (!outDocument || bytes.empty())
        return fail("empty VRMA input");
    ...
    if (!specVersion || !specVersion->IsString() || specVersion->GetString() != "1.0")
    {
        release();
    ...
    for (const auto& entry : *humanBones)
    {
        ...
        {
            outDocument->warnings.push_back("[VRMA101] ignored invalid humanoid bone mapping '" +
                                            entry.first + "'");
            continue;
        }
        boneNodes[*bone] = node;
    ...
    if (const JsObject* expressions = AsObject(Find(*root, "expressions")))
    {
        for (const char* group : {"preset", "custom"})
        {
            ...
            for (const auto& entry : *declared)
            {
                ...
                {
                    outDocument->warnings.push_back("[VRMA105] ignored expression '" + entry.first +
                                                    "' with no usable node");
                    continue;
                }
                bool isBone = false;
                ...
                {
                    outDocument->warnings.push_back("[VRMA106] ignored expression '" + entry.first +
                                                    "' whose node is already driven");
                    continue;
                }
                if (!expressionNames.insert(entry.first).second)
                {
                    outDocument->warnings.push_back("[VRMA107] ignored duplicate expression '" +
                                                    entry.first + "'");
                    continue;
                }
    ...
    const cgltf_node* lookAtNode = lookAtNodeIndex >= 0 ? &data->nodes[lookAtNodeIndex] : nullptr;
    ...
    for (cgltf_size channelIndex = 0; channelIndex != animation.channels_count; ++channelIndex)
    {
        ...
        if (jointIt == jointByNode.end() && expressionIt == expressionByNode.end() && !isLookAt)
        {
            continue;
        }
        if (channel.sampler->interpolation == cgltf_interpolation_type_cubic_spline && !warnedCubic)
        {
            outDocument->warnings.push_back(
        ...
        else if (channel.target_path == cgltf_animation_path_type_translation)
        {
            ...
            {
                outDocument->warnings.push_back("[VRMA103] ignored non-hips translation channel");
                continue;
            }
        }
    ...
    for (const auto& mapping : expressionByNodeIndex)
    {
        ...
        if (!node.has_translation && !node.has_matrix)
            continue;
        expression.constantWeight = static_cast<float>(NodeLocal(node).ExtractTranslation()[0]);
    }
    ...
    if (lookAtNode)
    {
        ...
        if (!outDocument->lookAt.isAnimated && StatesPlacement(*lookAtNode))
        {
            ...
            // withheld.
            const GfVec3d target = (NodeLocal(*lookAtNode) * lookAtAncestors).ExtractTranslation();
            outDocument->lookAt.constantTarget =
                GfVec3f(static_cast<float>(target[0]), static_cast<float>(target[1]),
    ...
    for (const float time : timeSet)
    {
        ...
        const auto report = [&](const std::string& name, float value)
        {
            ...
            {
                outDocument->warnings.push_back("[VRMA109] expression '" + name +
        ...
        if (lookAtSampler)
        {
            ...
            const GfVec3d target = lookAtAncestors.Transform(GfVec3d(value[0], value[1], value[2]));
            pose.lookAtTarget =
                GfVec3f(static_cast<float>(target[0]), static_cast<float>(target[1]),
                        static_cast<float>(target[2]));
        }
