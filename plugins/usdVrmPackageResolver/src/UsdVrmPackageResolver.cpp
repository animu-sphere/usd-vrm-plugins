...
const char*
_SniffImageExt(vrmContainer::ByteView bytes, const cgltf_image* img)
{
...
bool
_ReadPackageBytes(const std::string& packagePath, std::shared_ptr<const char>* outBuffer,
                  size_t* outSize)
{
    std::shared_ptr<ArAsset> asset = ArGetResolver().OpenAsset(ArResolvedPath(packagePath));
    if (!asset)
    {
        return false;
    }
...
_CopyImageBytes(vrmContainer::ByteView bytes)
{
    std::shared_ptr<char> image(new char[bytes.size()], std::default_delete<char[]>());
    if (!bytes.empty())
    {
...
std::shared_ptr<_PackageIndex>
_BuildPackageIndex(const std::string& packagePath)
{
    ...
    cgltf_result res = cgltf_parse(&options, packageBuffer.get(), packageSize, &data);
    if (res != cgltf_result_success || !data)
    {
        return index;
    }

    if (cgltf_load_buffers(&options, data, packagePath.c_str()) == cgltf_result_success)
    {
        for (cgltf_size i = 0; i < data->images_count; ++i)
        {
            ...
            index->images[vrmContainer::MakeEmbeddedResourcePath(imageBytes, ext)] =
                std::move(image);
        }
    }
...
_FindEmbeddedImage(const _PackageIndex& index, const std::string& packagedPath)
{
...
class UsdVrmPackageResolver final : public ArPackageResolver
{
  public:
    std::string
    Resolve(const std::string& resolvedPackagePath, const std::string& packagedPath) override
    {
        std::shared_ptr<const _PackageIndex> index = _FindOrBuildPackageIndex(resolvedPackagePath);
    ...
    {
        std::shared_ptr<const _PackageIndex> index = _FindOrBuildPackageIndex(resolvedPackagePath);
        const _EmbeddedImage* image =
            index ? _FindEmbeddedImage(*index, resolvedPackagedPath) : nullptr;
        if (!image)
        {
            return nullptr;
        }
    ...
    void
    BeginCacheScope(VtValue* cacheScopeData) override
    {
        _caches.BeginCacheScope(cacheScopeData);
    }

    void
    EndCacheScope(VtValue* cacheScopeData) override
    {
  ...
  private:
    std::shared_ptr<_PackageIndex>
    _FindOrBuildPackageIndex(const std::string& resolvedPackagePath)
    {
        ...
        std::shared_ptr<_PackageIndex> index = _BuildPackageIndex(resolvedPackagePath);
