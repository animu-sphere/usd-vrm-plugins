...
> **Historical only — archived 2026-09-25.** This was the boundary consolidation track (BND-0 to BND-5), and it was
...
> [current.md](../../roadmap/current.md), the plan in
...
| The **shape of a bake** — the rig's joint tokens beside the retargeted arrays, and one scale per joint (identity until the scale policy, each joint's rest scale since 2026-09-17) — is stated offline only in two lines of a **tool** (`StageIo.cpp`'s `WriteAnimation`), and `vrmRetarget` had no type for it. The library gained `JointLocalTransforms` for the registry; the rule stays two lines in the seam and two in the tool. Beside it, the tool rebuilds each sample's time code as `(timeCode / rate) × rate`, which at 30 fps bakes frame 62 at `62.00000000000001`. The smallest finding, and the first where the library gained the type and not the rule | `vrm.computeJointLocalTransforms` ([joint-transforms report §8, §10](../../reports/openusd/26.08-openexec-joint-transforms.md)) | the tool authoring each sample from the library's `JointLocalTransforms`, at the time code it read rather than one rebuilt from seconds, so P1-2's "OpenExec and offline behave identically" is one statement |
| **Which prim a stage means** is decided only in a **tool**: the humanoid is the first prim stating `vrm:humanBones:hips`, the rig is `--skeleton`, else that humanoid's `vrm:skeleton`, else the first skeleton on the stage, and the clip is the first skeleton on its stage and its binding, else the stage's one animation (`StageIo.cpp`'s `ReadAvatar` and `ReadClip`). The parity harness restates the first three to compare the same input, and refuses the last. The sampler's finding a fourth time, one level up: not how to read a value, but which prim to read it from. Found by the first consumer that is a test rather than a node, and the first to need both halves of a bake at once. Beside it, **diagnostics had no value form**: `PoseRetargeter` reported a dropped clip bone into `RetargetDiagnostics` as a bone list and a sentence, which a computation cannot answer, so exec could not say what the tool says. *Since 2026-09-13 they do* (P1-1's first half): frozen `VRM_RETARGET_*` codes as comparable values, and `DiagnoseRig` for a caller that retargets one pose at a time. *And exec answers them the same day*: two `execVrm` nodes, and the harness compares their list with the tool's line for line, equal on every case ([diagnostics report §2](../../reports/openusd/26.08-openexec-diagnostics.md)) | P0-6's harness ([parity report §6, §8](../../reports/openusd/26.08-openexec-parity.md)) | one statement of the selection rules that the tool and BND-1's reference pipeline both call, or the decision that the stage names every one of them, which is the direction exec's relationships already point. The diagnostics half is closed; its cost is the row below |
| A pose's diagnostics exist only as a **side effect of retargeting it**: `PoseRetargeter::Retarget(pose, &diagnostics)` reports what the pose drives that the rig does not bind, and there is no call that reports it without retargeting. A computation answers one value, so `vrm.computeRetargetDiagnostics` repeats the retarget, correction included, beside `vrm.humanoidRetarget`. Measured with both asked for: **23 µs a frame** on the 23-joint fixture rig and **50–60 µs** on Seed-san's 128, of which a cached second `Compute` is 4–6 µs. And because it retargets the stage's sample rather than reading the retarget's answer, a driver's override of the retarget is not diagnosed. The eighth finding's kind, one method over | `vrm.computeRetargetDiagnostics` ([diagnostics report §6](../../reports/openusd/26.08-openexec-diagnostics.md)) | a per-pose counterpart to `DiagnoseRig` — the rule `Retarget` applies, reachable without the retarget — that `Retarget` itself calls, so the rule has one implementation and the node wraps it |
...
| `motion:timeCodesPerSecond`, `motion:filter:*` and `motion:root:intake` are authored by fixtures and by nothing else, and so are `execVrm`'s `vrm:retarget:sourceSkeleton` and its four `vrm:retarget:*` root-motion statements. Since P0-7 the display route adds two more: a clip's declared `motion:root:transform`, and the connection a prop's `xformOp:transform` makes to it ([display report §9](../../reports/openusd/26.08-openexec-display.md)). Since P0-6 the parity harness authors the rate, the relationship and the four statements too, onto the stage it compares, and with them the two implementations agree bit for bit ([parity report §5](../../reports/openusd/26.08-openexec-parity.md)); that makes the harness their one consumer-side author, not a producer | the first three nodes, `vrm.computeRestPoseCorrection` and `vrm.humanoidRetarget` | BND-0 below decides whether a rate shim and two policies belong on a clip at all, and whether the stage or a producer says which clip drives which humanoid and where its root lands. *Proposed 2026-09-19* in usd-motion-plugins' `EXEC_CONTRACT.md` §5: the rate is every motion writer's, as a shim; the policies, the placement, the blend fan-in and the retarget binding are the composed scene's |
...
*A starting point exists since 2026-09-13*: `tests/parity/` runs a BVH export
and a `.vrma` clip through one downstream call sequence twice, once offline and
once through exec, and compares the two
([the parity report](../../reports/openusd/26.08-openexec-parity.md)). It is not
this test. It proves two implementations of the downstream half agree, not that
every source reaches it the same way, and it has no recorded live trace. What
it can lend is its stage composition and the selection rules the tenth finding
above names.
