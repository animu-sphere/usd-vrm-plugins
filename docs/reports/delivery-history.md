...
- ✅ **Motion Phase A — the contract** (v0.3.0). `motionCore`: vendor-neutral
  `MotionPose` / `MotionClip` / `RootMotion` / `MotionConstraintSet` and
...
- ✅ **Motion Phase D — live capture** (v0.5.0). `IMotionSource` / `ClipSource` /
  ...
  `MotionRecorder`, and the `motion_capture` CLI. A captured session is baked
...
- ✅ **One humanoid taxonomy** (v0.5.0). `HumanBoneParent`,
  `NearestPresentAncestor` and `HumanJointPath` moved into `motionCore`; the
...
- ✅ **`vrmAdapterVmc`**: a plain static library that keeps VMC Protocol at an
  ...
  maps Unity bone names and axes into `motion::HumanJoint`, assembles frames, and
...
- ✅ **`libs/motionTracking`**: which tracker is which body region, without
  ...
  observations reach a `MotionPose` by direct assignment, and a position the
