...
| [26.08-openexec-root-motion.md](26.08-openexec-root-motion.md) | 26.08 | OpenExec value types and drivers, **run**: that a bundle registers more than one value type and a computation may answer in a type other than the one it reads, that time dependence follows the link rather than the type, that an **undeclared** input is silent, and that one override drives every node depending on it. Extends the filtering report and replaces no audit. |
