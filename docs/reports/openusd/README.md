...
| [26.08-openexec-parity.md](26.08-openexec-parity.md) | 26.08 | OpenExec against the offline bake, **run**: that `execMotion` + `execVrm` answer the same recorded input as `motion_retarget` **bit for bit**, on a fixture rig, on `Seed-san.vrm`, under every root-motion statement and through the VRMA reader; that the tool's time-code rounding is placement only and vanishes when a bake is compared at its own samples; what the harness had to state on the stage for exec to be asked the same question; and that the two sides' **diagnostics** do not agree. Collects the rows four earlier reports left for P0-6 and replaces no audit. |
