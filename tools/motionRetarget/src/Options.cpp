...
GetUsage()
{
    return "motion_retarget - bake a semantic humanoid clip onto a target rig\n"
           "\n"
           "Usage:\n"
             ...
             std::string* error)
{
    *showHelp = false;
    for (std::size_t i = 0; i < arguments.size(); ++i)
    {
        ...
        {
            if (!TakeValue(arguments, &i, argument, &options->animationPath, error))
            {
                return false;
            }
        }
        ...
        {
            if (!TakeValue(arguments, &i, argument, &options->humanoidMapPath, error))
            {
                return false;
            }
        }
        ...
        {
            if (!TakeValue(arguments, &i, argument, &options->targetSkeletonPath, error))
            {
                return false;
            }
        }
        ...
        {
            if (!TakeValue(arguments, &i, argument, &options->clipSkeletonPath, error))
            {
                return false;
            }
        }
        ...
        {
            if (!TakeValue(arguments, &i, argument, &options->animationName, error))
            {
                return false;
            }
        }
        else if (argument == "--root-motion")
        {
            ...
            {
                *error = "--root-motion expects hips, root, or ignore; got '" + mode + "'";
                return false;
            }
        }
        else if (argument == "--root-joint")
        {
            if (!TakeValue(arguments, &i, argument, &options->rootJointToken, error))
            {
                return false;
            }
        }
        ...
        {
            if (!TakeDouble(arguments, &i, argument, &options->resampleRate, error))
            {
                return false;
            }
