             ...
             std::string* error)
{
    *showHelp = false;
    for (std::size_t i = 0; i < arguments.size(); ++i)
    {
        ...
        {
            if (!TakeValue(arguments, &i, argument, &options->tracePath, error))
            {
                return false;
            }
        }
        ...
        {
            if (!TakeDouble(arguments, &i, argument, &options->evaluationRate, error))
            {
                return false;
            }
            if (options->evaluationRate <= 0.0 || options->evaluationRate > kMaxEvaluationRate)
            {
                *error = "--rate expects a positive frequency no greater than " +
                         std::to_string(static_cast<long long>(kMaxEvaluationRate));
                return false;
            }
        }
        else if (argument == "--delivery-lag")
        {
            if (!TakeDouble(arguments, &i, argument, &options->deliveryLag, error))
            {
                return false;
            }
        ...
        else if (argument == "--buffer-capacity")
        {
            ...
            if (capacity < 1.0 || capacity > kMaxBufferCapacity || capacity != std::floor(capacity))
            {
                *error = "--buffer-capacity expects a whole number of frames "
                         "between 1 and " +
                         std::to_string(static_cast<long long>(kMaxBufferCapacity));
                return false;
            }
            options->capture.bufferCapacity = static_cast<std::size_t>(capacity);
        }
    ...
    if (options->outputPath.empty() && options->normalizePath.empty() && !options->dryRun)
    {
    ...
    if (!options->normalizePath.empty())
    {
        ...
        if (conflict)
        {
            *error = std::string("--normalize rewrites the trace and exits, so ") + conflict +
