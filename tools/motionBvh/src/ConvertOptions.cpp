...
bool
TakeSize(const std::vector<std::string>& arguments, std::size_t* index, const std::string& flag,
...
bool
TakePositive(const std::vector<std::string>& arguments, std::size_t* index, const std::string& flag,
             std::size_t* value, std::string* error)
{
...
bool
ParseConvertOptions(const std::vector<std::string>& arguments, ConvertOptions* options,
                    bool* showHelp, std::string* error)
{
    *showHelp = false;
    for (std::size_t i = 0; i < arguments.size(); ++i)
    {
        ...
        {
            if (!TakePositive(arguments, &i, argument, &options->limits.maxHierarchyDepth, error))
            {
                return false;
            }
        }
        ...
        {
            if (!TakePositive(arguments, &i, argument, &options->limits.maxJoints, error))
            {
                return false;
            }
        }
        else if (argument == "--max-frames")
        {
            ...
            // legal.
            if (!TakeSize(arguments, &i, argument, &options->limits.maxFrames, error))
            {
                return false;
            }
        }
        else if (argument == "--")
        {
            ...
            {
                *error =
                    "this tool converts one file at a time; '" + arguments[i] + "' is a second";
                return false;
            }
            ...
            {
                *error =
                    "this tool converts one file at a time; '" + arguments[i + 1] + "' is a second";
                return false;
            }
        }
        ...
        {
            *error = "this tool converts one file at a time; '" + argument + "' is a second";
            return false;
        }
    }
